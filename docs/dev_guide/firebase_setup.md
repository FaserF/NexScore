...
<script>
  firebase.initializeApp({
    ...
    storageBucket: "nexscore-app.firebasestorage.app",
...
|---|---|---|
| `FIREBASE_API_KEY` | `...` | Web / Docker / PWA build |
| `FIREBASE_AUTH_DOMAIN` | `nexscore-app.firebaseapp.com` | Web / Docker / PWA build |
| `FIREBASE_PROJECT_ID` | `nexscore-app` | Web / Docker / PWA build |
| `FIREBASE_STORAGE_BUCKET` | `nexscore-app.firebasestorage.app` | Web / Docker / PWA build |
| `FIREBASE_MESSAGING_SENDER_ID` | `96410867697` | Web / Docker / PWA build |
| `FIREBASE_APP_ID` | `...` | Web / Docker / PWA build |
| `FIREBASE_MEASUREMENT_ID` | `...` | Web Analytics |
| `GOOGLE_SERVICES_JSON_BASE64` | `...` | Android native build (Base64) |
| `GOOGLE_SERVICE_INFO_PLIST_BASE64` | `...` | iOS native build (Base64) |
...
- name: Decode google-services.json
  run: echo "${{ secrets.GOOGLE_SERVICES_JSON_BASE64 }}" | base64 -d > nexscore/android/app/google-services.json
```
...
- name: Decode GoogleService-Info.plist
  run: echo "${{ secrets.GOOGLE_SERVICE_INFO_PLIST_BASE64 }}" | base64 -d > nexscore/ios/Runner/GoogleService-Info.plist
```
...
## 5.4 High-Level Verification Checklist

To ensure **Multiplayer** works on your GitHub Pages PWA, verify this list:

1. [ ] **GitHub Secrets**: All 6 `FIREBASE_...` secrets are set in your repo settings.
2. [ ] **Firebase Auth**: Google and GitHub providers are enabled in Firebase Console.
3. [ ] **Authorized Domains**: `faserf.github.io` (or your domain) is added to Firebase Auth settings.
4. [ ] **GitHub OAuth**: The callback URL in GitHub matches your Firebase `__/auth/handler` exactly.
5. [ ] **Firestore**: Database is created and "Multiplayer" collection exists (or rules allow auto-creation).

> [!TIP]
> Use the **"Diagnostic Dialog"** in the NexScore PWA (found in the Multiplayer room creation screen) to check if the app detected your configuration correctly.
